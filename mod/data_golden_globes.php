...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    try {
        ...
        $stmt = $pdo->prepare('DELETE FROM golden_globe_nominations WHERE id = ?');
        $stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
    try {
        ...
        $yearFilm = (int)$_POST['year_film'];
        $yearAward = (int)$_POST['year_award'];
        $ceremony = (int)$_POST['ceremony'];
        ...
        if (empty($nominee)) {
            throw new Exception('Nominee darf nicht leer sein.');
        }
        
        if ($id) {
            // Update
            $stmt = $pdo->prepare('
                UPDATE golden_globe_nominations 
                SET year_film = ?, year_award = ?, ceremony = ?, category_id = ?, 
                    nominee = ?, film = ?, imdb_const = ?, winner = ?
                WHERE id = ?
            ');
            $stmt->execute([$yearFilm, $yearAward, $ceremony, $categoryId, $nominee, $film, $imdbConst, $winner, $id]);
            ...
            $stmt = $pdo->prepare('
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ');
            $stmt->execute([$yearFilm, $yearAward, $ceremony, $categoryId, $nominee, $film, $imdbConst, $winner]);
            $message = '✓ Neuer Eintrag wurde angelegt. Formular bleibt für weiteren Eintrag geöffnet.';
            
            // Formular bleibt offen mit den eingegebenen Daten (außer nominee und film werden geleert)
            $_GET['new'] = '1';
        ...
        if (!$id) {
            $editEntry = [
                ...
                'nominee' => '',  // Leeren für nächsten Eintrag
...
if (isset($_GET['edit']) && !isset($_POST['save'])) {
    ...
    if ($editId > 0) {
        $stmt = $pdo->prepare('SELECT * FROM golden_globe_nominations WHERE id = ?');
        $stmt->execute([$editId]);
...
// Kategorien laden
$categories = [];
$stmtCat = $pdo->query('SELECT id, name, german FROM golden_globe_category ORDER BY COALESCE(german, name)');
...
// Gesamtanzahl
$countSql = "SELECT COUNT(*) FROM golden_globe_nominations gg $whereClause";
if (!empty($params)) {
...
// Daten laden
$sql = "SELECT gg.*, COALESCE(gc.german, gc.name) AS category_name
...
// Jahre für Filter
$years = $pdo->query('SELECT DISTINCT year_award FROM golden_globe_nominations ORDER BY year_award DESC')->fetchAll(PDO::FETCH_COLUMN);
...
<div class="row">
    <div class="col-12">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h2>🎭 Golden Globe Daten bearbeiten</h2>
            <a href="?mod=data_golden_globes&new" class="btn btn-sm btn-success">+ Neuer Eintrag</a>
        </div>
            ...
            <div class="card mb-4" style="background: var(--card-bg); border-color: var(--table-border);">
                <div class="card-body">
                    ...
                    <form method="POST">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-3">
                                <label class="form-label">Jahr (Film)</label>
                                <input type="number" class="form-control" name="year_film" value="<?php echo h($editEntry['year_film']); ?>" required>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Jahr (Auszeichnung)</label>
                                <input type="number" class="form-control" name="year_award" value="<?php echo h($editEntry['year_award']); ?>" required>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Zeremonie Nr.</label>
                                <input type="number" class="form-control" name="ceremony" value="<?php echo h($editEntry['ceremony']); ?>" required>
                            </div>
                            <div class="col-md-3">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" name="nominee" value="<?php echo h($editEntry['nominee']); ?>" required>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Film</label>
                                <input type="text" class="form-control" name="film" value="<?php echo h($editEntry['film']); ?>">
                            </div>
                        </div>
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <a href="?mod=data_golden_globes" class="btn btn-secondary">Abbrechen</a>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <form class="d-flex mb-3" method="get" style="gap:.5rem">
            <input type="hidden" name="mod" value="data_golden_globes">
            <input class="form-control form-control-sm" type="search" name="q" placeholder="Suche Nominee/Film" value="<?php echo h($searchTerm); ?>" style="width: 300px;">
            ...
            <?php if ($searchTerm || $yearFilter || $categoryFilter): ?>
                <a href="?mod=data_golden_globes" class="btn btn-sm btn-secondary">Zurücksetzen</a>
            <?php endif; ?>
        </form>
                    ...
                    <?php else: ?>
                        <?php foreach ($entries as $entry): ?>
                            <tr>
                                ...
                                <td><?php echo h($entry['year_award']); ?></td>
                                <td style="font-size: 0.9em;"><?php echo h($entry['category_name']); ?></td>
                                <td><?php echo h($entry['nominee']); ?></td>
                                <td><?php echo !empty($entry['film']) ? h($entry['film']) : '<span style="opacity:0.5;">—</span>'; ?></td>
                                <td>
                                ...
                                <td>
                                    <a href="?mod=data_golden_globes&edit=<?php echo $entry['id']; ?>" class="btn btn-sm btn-outline-primary">✏️ Bearbeiten</a>
        ...
        <?php if ($pages > 1): ?>
            <nav aria-label="Seiten" class="mt-4">
                <ul class="pagination justify-content-center">
                    <?php
                    $baseUrl = '?mod=data_golden_globes';
